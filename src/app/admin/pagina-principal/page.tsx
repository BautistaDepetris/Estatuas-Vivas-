...
export default function AdminPaginaPrincipalPage() {
    ...
    useEffect(() => {
        async function cargarDatos() {
            ...
            setFotos(!galeriaError && galeria?.length
                ? galeria.map((foto) => ({
                    id: String(foto.id),
                    url: foto.url,
                    titulo: foto.titulo,
                ...
                : fotosMock());
            setLugares(!lugaresError && lugaresPueblo?.length
      ...
      <div>
        ...
        <section>
          <div>
            ...
            <span>{lugares.length} lugares</span>
          </div>
          ...
          <div>
            {lugares.map((lugar) => (<article key={lugar.id}>
                ...
                <div>
                  <button onClick={() => editarLugar(lugar)}>Editar</button>
          ...
          <form onSubmit={guardarLugar}>
            <p>{editandoLugarId ? 'Editar lugar' : 'Agregar lugar nuevo'}</p>
            <div>
              ...
              <label>Subir imagen<input required={!editandoLugarId} type="file" accept="image/*" onChange={(e) => setLugarForm((prev) => ({ ...prev, archivo: e.target.files?.[0] ?? null }))}/></label>
            </div>
            {(lugarPreview || lugarForm.imagen_url) && (<div>
                <p>Preview</p>
                <img src={lugarPreview || lugarForm.imagen_url || ''} alt="Preview del lugar"/>
              </div>)}
            <button type="submit">Guardar</button>
          </form>
        </section>
