...
export default function AdminEstatuaEditorPage({ params }: AdminEstatuaEditorPageProps) {
    ...
    useEffect(() => {
        async function cargarEstatua() {
            ...
            setCapitulos(completarSieteCapitulos(capitulosDb?.length
    ...
    return (<section>
      ...
      <div>
        <section>
          <div>
            <h2>Los 7 Capítulos</h2>
            <button onClick={guardarCapitulos}>Guardar sección</button>
          </div>
          <div>
            {capitulos.map((capitulo, index) => (<article key={`${capitulo.id}-${index}`}>
                <p>Capítulo {index + 1}</p>
                <div>
                  <input required value={capitulo.titulo} onChange={(event) => setCapitulos((prev) => prev.map((item, itemIndex) => itemIndex === index ? { ...item, titulo: event.target.value } : item))}/>
                  <textarea required value={capitulo.texto} onChange={(event) => setCapitulos((prev) => prev.map((item, itemIndex) => itemIndex === index ? { ...item, texto: event.target.value } : item))}/>
                </div>
              </article>))}
          </div>
        </section>
        ...
        <section>
          <div>
            ...
            <button onClick={guardarFrase}>Guardar</button>
          </div>
          <textarea required value={frase} onChange={(event) => setFrase(event.target.value)}/>
        </section>
        ...
        <section>
          ...
          <form onSubmit={agregarFoto}>
            <p>Agregar foto</p>
            <div>
              <label>Subir imagen<input required type="file" accept="image/*" onChange={(e) => setFotoForm((prev) => ({ ...prev, archivo: e.target.files?.[0] ?? null }))}/></label>
              <label>Título<input required value={fotoForm.titulo} onChange={(e) => setFotoForm((prev) => ({ ...prev, titulo: e.target.value }))}/></label>
              <label>Descripción<input required value={fotoForm.descripcion} onChange={(e) => setFotoForm((prev) => ({ ...prev, descripcion: e.target.value }))}/></label>
              <label>Categoría<input required value={fotoForm.categoria} onChange={(e) => setFotoForm((prev) => ({ ...prev, categoria: e.target.value }))}/></label>
            </div>
            {fotoPreview && (<div>
                <p>Preview</p>
        ...
        <section>
          ...
          <div>
            {lugares.map((lugar) => (<article key={lugar.id}>
                ...
                <button onClick={() => eliminarLugar(lugar)}>Eliminar</button>
              </article>))}
          </div>
          <form onSubmit={agregarLugar}>
            <p>Agregar lugar</p>
            <div>
              <label>Nombre<input required value={lugarForm.nombre} onChange={(e) => setLugarForm((prev) => ({ ...prev, nombre: e.target.value }))}/></label>
              <label>Descripción<input required value={lugarForm.descripcion} onChange={(e) => setLugarForm((prev) => ({ ...prev, descripcion: e.target.value }))}/></label>
              <label>Categoría<input required value={lugarForm.categoria} onChange={(e) => setLugarForm((prev) => ({ ...prev, categoria: e.target.value }))}/></label>
              <label>Subir imagen<input required type="file" accept="image/*" onChange={(e) => setLugarForm((prev) => ({ ...prev, archivo: e.target.files?.[0] ?? null }))}/></label>
            </div>
            {lugarPreview && (<div>
                <p>Preview</p>
                <img src={lugarPreview} alt="Preview del lugar recomendado"/>
              </div>)}
            <button type="submit">Guardar</button>
          </form>
        </section>
